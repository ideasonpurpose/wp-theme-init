...
namespace ideasonpurpose\Debug;
...
class ShowIncludes
{
    ...
    public function getFiles()
    {
        ...
        $this->templates = array_map(function ($t) {
            global $template;
            $t = ($t == $template) ? "<span class='template' style='font-weight: bold; color: #000;'>$t</span>" : $t;
            return '<li style="list-style-type: none; color: #888; margin: 0;">' . str_replace(get_template_directory(), '', $t) . '</li>';
        }, $templates);

        global $template;
        $this->templateName = str_replace(get_template_directory(), '', $template);
    }
    ...
    public function show()
    {
        ...
        <div class="container" id="iop-wp-debug-template" style="font-size: 14px; color: #400; overflow: auto; margin: 3em auto; padding: 15px; border: 1px solid #ccc; border-radius: 4px; background: #eee; max-width: 90%;">
          <strong>Template: <span style="font-family: monospace; font-size: 1.1em;"><?= $this->templateName ?></span></strong><br>
          Included Files:
